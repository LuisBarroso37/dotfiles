...
| F-14 | ~~The nvim pty probe failed 3/3 after a batch of killed nvim processes, then passed 8/8 from a clean state. Dirty prior TUI state.~~ **This diagnosis was wrong** — it was a guess that fit the timing, recorded as if it were a finding. The real cause is F-17, proven later the same day: 3/3 fail and 3/3 pass depending only on stdin. Kept visible rather than deleted, because a plausible story that matches the timing is exactly how a wrong diagnosis survives. | **SUPERSEDED by F-17 (2026-08-01)** |
| F-15 | The GitHub token is passed as `-H "Authorization: Bearer …"`, so it appears in the process argument list and is readable from `/proc/<pid>/cmdline` by other local users. Real mechanism, negligible impact on a single-user machine; `curl --config -` would keep it off argv. | **FIXED (2026-08-01)** — a `gh_api_get` helper feeds the header in on stdin via `--config -`. Proven by inspecting a live curl's argv: `Bearer` is present with `-H`, absent with `--config -`. Fixed on request, against this file's own criteria, which had scored the cost today as nil |

| F-16 | **A gate failure nobody could attribute.** `./check.sh --offline` reported `FAIL — 1 failure(s)` once, then passed 10 consecutive runs. The cause was initially unknowable because that run had been read through `tail -3`, and the `✗` line naming the check had scrolled past — the gate had said what broke and nobody was looking. | FIXED (2026-08-01) — `check.sh` restates the failed checks under the FAIL verdict, so a truncated read is self-sufficient. It paid for itself immediately: the very next occurrence named itself as `zsh -i exited 1`, which produced F-17 |
| F-17 | **The gate could not be run non-interactively.** macOS `script(1)` calls `tcgetattr` on its **own stdin** before running anything, so when it inherits an fd that is not terminal-like — a socket, as under an agent harness, cron, or some CI runners — it exits 1 with `tcgetattr/ioctl: Operation not supported on socket` having never started the command. Both pty probes then reported the *thing being probed* as broken: C2 said `zsh -i exited 1` on a healthy shell, and the nvim probe silently produced no output. It presented as intermittent only because whether stdin was a socket varied per invocation — `/dev/null` and pipes both work, so most runs passed. Root cause of both F-14 and F-16. | **FIXED (2026-08-01)** — `in_pty` pins its stdin to `/dev/null`, making both probes independent of how they were invoked. Proven: nvim probe 3/3 fail without, 3/3 pass with; C2 5/5 pass under the exact socket-stdin condition that had just failed |
